...
#define GET_NEXT(p)    ((BlockHeader *)((uintptr_t)((p)->next) & ~0x1))   /* TODO: Mask out free flag */
#define SET_NEXT(p,n)  (p->next = (BlockHeader *)((uintptr_t)(n) | ((uintptr_t)(p->next) & 0x1)))  /* TODO: Preserve free flag */
#define GET_FREE(p)    (uint8_t) ( (uintptr_t) (p->next) & 0x1 )   /* OK -- do not change */
#define SET_FREE(p,f)  (p->next = (BlockHeader *)(((uintptr_t)GET_NEXT(p)) | ((f) ? 0x1 : 0x0))) /* TODO: Set free bit of p->next to f */
#define SIZE(p)        ((size_t)((uintptr_t)GET_NEXT(p) - (uintptr_t)(p->user_block))) /* TODO: Caluculate size of block from p and p->next */ 
...
void* simple_malloc(size_t size) {
  ...
  size_t aligned_size = (size + 7) & ~0x7;  /* TODO: Alignment */
  ...
  do {
    ...
    if (GET_FREE(current)) {
      ...
      if (SIZE(current) >= aligned_size) {
        ...
        BlockHeader *alloc = current;

        /* Will the remainder be large enough for a new block? */
        if (SIZE(alloc) - aligned_size < sizeof(BlockHeader) + MIN_SIZE) {
          /* TODO: Use block as is, marking it non-free*/
          SET_FREE(alloc, 0);
        } else {
          /* TODO: Carve aligned_size from block and allocate new free block for the rest */
          BlockHeader *new = (BlockHeader *)((uintptr_t)alloc->user_block + aligned_size);
          new->next = GET_NEXT(alloc);
        ...
        current = GET_NEXT(alloc);
        return (void *)alloc->user_block; /* TODO: Return address of current's user_block and advance current */
      }
    }
...
void simple_free(void * ptr) {
  ...
  while(GET_FREE(GET_NEXT(current)) && GET_NEXT(current) != current){
    BlockHeader *next_free_block = GET_NEXT(current);

    SET_NEXT(current, GET_NEXT(next_free_block));
  }
